...
class PruningPolicy(abc.ABC):
  ...
  basis as well as checks that the model contains only supported layers.
...
class PruneForLatencyOnXNNPack(PruningPolicy):
  ...
  def _lookup_layers(self, start_layers, stop_fn, next_fn):
    """Traverses the model and returns layers satisfied `stop_fn` criteria."""
    to_visit = set(start_layers)
    used_layers = set(start_layers)
    found_layers = set()
    while to_visit:
      layer = to_visit.pop()
      if stop_fn(layer):
