...
class Conv2DBatchNormQuantize(transforms.Transform):
  ...
  def custom_objects(self):
    return {
        'NoOpQuantizeConfig': ternarization_quantize_configs.NoOpQuantizeConfig,
...
class Conv2DReshapeBatchNormReLUQuantize(Conv2DBatchNormReLUQuantize):
  ...
  def pattern(self):
    return LayerPattern(
        'ReLU', inputs=[Conv2DReshapeBatchNormQuantize.pattern(self)])
...
class SeparableConv1DQuantize(transforms.Transform):
  ...
  def replacement(self, match_layer):
    ...
    expand_layer_metadata = {
        'quantize_config': ternarization_quantize_configs.NoOpQuantizeConfig()
    }
...
class ConcatTransform(transforms.Transform):
  ...
  def pattern(self):
    ...
    return LayerPattern(
        'Concatenate', inputs=[LayerPattern('.*'),
                               LayerPattern('.*')])
