...
class QuantizeNumericalTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def testModelEndToEnd(self, model_fn):
    ...
    # 3. Check if the weights are ternarized.
    # For now disabling this since it's related with loss config and epoch.
    # for layer in model.layers:
    #  weights = layer.get_weights()
    #  for weight in weights:
    #    for w in np.nditer(weight):
    #      self.assertTrue(
    #          np.isclose(w, -1) or np.isclose(w, 0) or np.isclose(w, 1))

    x_test, y_test = self._create_test_data(model)

    y_tf = model.predict(x_test)

    # 4. Ensure conversion to TFLite works.
    _, tflite_file = tempfile.mkstemp('.tflite')
    print('TFLite File: ', tflite_file)
    with quantize.quantize_scope():
      utils.convert_keras_to_tflite(model, tflite_file)

    # 5. Verify input runs on converted model.
    y_tfl = self._execute_tflite(tflite_file, x_test, y_test)

    # 6. Verify results are the same in TF and TFL.
