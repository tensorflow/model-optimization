...
class AbstractClusteringAlgorithm(object):
  """
  The reason to have an abstract class here is to be able to implement highly-efficient vectorised look-ups.

  We do not utilise looping for that purpose, instead we `smartly` reshape and tile arrays. The trade-off is that we
  are potentially using way more memory than we would have if looping is used.
  ...
  def __init__(self, clusters_centroids):
    """
    For generating clustered tensors we will need two things: cluster centroids and the final shape tensor must have.
    :param clusters_centroids: An array of shape (N,) that contains initial values of clusters centroids.
    """
  ...
  def get_pulling_indices(self, weight):
    """
    Takes a weight(can be 1D, 2D or ND) and creates tf.int32 array of the same shape that will hold
    indices of cluster centroids clustered arrays elements will be pulled from.

    In the current setup pulling indices are meant to be created once and used everywhere
    :param weight: ND array of weights. For each weight in this array the closest cluster centroids is found.
    :return: ND array of the same shape as `weight` parameter of the type tf.int32. The returned array contain weight
    lookup indices
    """
    pass
  ...
  def get_clustered_weight(self, pulling_indices):
    """
    Takes an array with integer number that represent lookup indices and forms a new array according to
    the given indices.
    :param pulling_indices: an array of indices used for lookup.
    :return: array with the same shape as `pulling_indices`. Each array element is a member of self.cluster_centroids
    """
    return tf.reshape(tf.gather(self.cluster_centroids, tf.reshape(pulling_indices, shape=(-1,))),
...
class ConvolutionalWeightsCA(AbstractClusteringAlgorithm):
  ...
  def get_pulling_indices(self, weight):
    ...
    tiled_cluster_centroids = tf.stack(
      [tf.tile(tf.stack([tf.reshape(self.cluster_centroids, [1, 1, clst_num])] * weight.shape[-2], axis=2),
               [weight.shape[0], weight.shape[1], 1, 1])] * weight.shape[-1], axis=3)

    # We find the nearest cluster centroids and store them so that ops can build their kernels upon it
    pulling_indices = tf.argmin(tf.abs(tiled_weights - tiled_cluster_centroids), axis=4)
...
class DenseWeightsCA(AbstractClusteringAlgorithm):
  ...
  def get_pulling_indices(self, weight):
    ...
    tiled_cluster_centroids = tf.tile(tf.reshape(self.cluster_centroids, [1, 1, clst_num]),
...
class BiasWeightsCA(AbstractClusteringAlgorithm):
  ...
  def get_pulling_indices(self, weight):
    ...
    tiled_cluster_centroids = tf.tile(tf.reshape(self.cluster_centroids, [1, clst_num]), [weight.shape[0], 1])

    pulling_indices = tf.argmin(tf.abs(tiled_weights - tiled_cluster_centroids), axis=1)
...
class ClusteringLookupRegistry(object):
  """
  The keys represent built-in keras layers and the values represent the strategy accoding to which clustering will be
  done.
  If the key is not present in the map, that means that there is nothing to work on, or the strategy is not currently
  supported
  """
  _LAYERS_RESHAPE_MAP = {
    ...
    layers.convolutional.DepthwiseConv2D: {'depthwise_kernel': ConvolutionalWeightsCA},
    layers.convolutional.SeparableConv1D: {'pointwise_kernel': ConvolutionalWeightsCA},
    layers.convolutional.SeparableConv2D: {'pointwise_kernel': ConvolutionalWeightsCA},
  ...
  def register_new_implementation(cls, new_impl):
    """
    For custom user-defined objects define the way how clusterable weights are going to be formed. If weights are any of
    these, 1D,2D or 4D, please consider using existing implementations: BiasWeightsCA, ConvolutionalWeightsCA and
    DenseWeightsCA.

    :param new_impl: dictionary. Keys are classes and values are dictionaries. The latter have strings as keys and values
    are classes inherited from AbstractClusteringAlgorithm. Normally, the set keys of the latter dictionaries should
    match the set of clusterable weights names for the layer.
    :return: None
    """
    ...
    for k, v in new_impl.items():
      if not isinstance(v, dict):
        raise TypeError("Every value of new_impl must be a dictionary. Item for key {key} has class {vclass}".format(
          key=k,
          vclass=v
        ))
  ...
  def get_clustering_impl(cls, layer, weight_name):
    ...
    if not layer.__class__ in cls._LAYERS_RESHAPE_MAP:
      raise ValueError("Class {given_class} has not been registerd in the ClusteringLookupRegistry.\
Use ClusteringLookupRegistry.register_new_implemenetation to fix this.".format(given_class=layer.__class__))
    if weight_name not in cls._LAYERS_RESHAPE_MAP[layer.__class__]:
      raise ValueError("Weight with the name '{given_weight_name}' for class {given_class} has not been registerd in the \
ClusteringLookupRegistry.Use ClusteringLookupRegistry.register_new_implemenetation to fix this.".format(
        given_class=layer.__class__,
        given_weight_name=weight_name))
    # Different weights will have different shapes hence there is double hash map lookup.
...
class ClusteringRegistry(object):
  ...
  def make_clusterable(cls, layer):
    ...
    def get_clusterable_weights():
      return [(weight, getattr(layer, weight)) for weight in cls._weight_names(layer)]
