...
class FunctionalTest(tf.test.TestCase):
  ...
  def testMnist(self):
    ...
    # check the sparsity is greater than 10%
    import numpy as np
    kernel = clustered_model.layers[-1].kernel.numpy()
    nb_zero_in_kernel = kernel.size - np.count_nonzero(kernel)
    self.assertGreater(nb_zero_in_kernel, kernel.size * 0.1)
