...
from tensorflow_model_optimization.python.core.tf2_sparsity.keras import pruning_schedule as pruning_sched
...
class Pruner(object):
  ...
  def _update_mask(self, step, weights):
    ...
    with tf.name_scope('pruning_ops'): # TODO(xwinxu): remove scopes?
  ...
  def update_masks(self, pruning_vars, step):
    ...
    # raise ValueError(str(tf.__version__))
